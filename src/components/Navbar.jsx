...
<button type='button' onClick={customFunc} style={{color}} className="relative text-xl rounded-full p-3 hover:bg-light-gray">{icon}
...
//useEffect for handling screensize dynamically
useEffect(()=>{

  const handleResize= () => setScreenSize(window.innerWidth);
  //adding event lisener to resize screen 
  window.addEventListener('resize', handleResize);
  //handle inital width
  handleResize();
  //removes event lisener after use
 ...
 { setActiveMenu(true);
  ...
  return (
    <div className='flex justify-between p-2 md:mx-6 relative'>
      ...
      <NavButton title='Notifacations' dotcolor='#03C9D7' customFunc={() => handleClick('notification')} color="blue" icon={<RiNotification3Line />} />
